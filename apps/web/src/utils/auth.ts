...
import { ROLES_ENUM, teamMembershipTable, userTable } from "@/db/schema";
...
import { selectActiveTeamId } from "@/lib/team-session";
...
export async function createSession({
  ...
  const activeTeamId = selectActiveTeamId({
    defaultTeamId: user.defaultTeamId,
    memberships,
  });

  if ((user.defaultTeamId ?? undefined) !== activeTeamId) {
    await db
      .update(userTable)
      .set({ defaultTeamId: activeTeamId ?? null })
      .where(eq(userTable.id, userId));
  }
