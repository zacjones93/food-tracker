const SERIALIZED_CODE_TOOL_CALL = /\{\s*"name"\s*:\s*"codemode_execute"\s*,\s*"parameters"\s*:\s*\{\s*"code"\s*:\s*"(?:\\.|[^"\\])*"\s*\}\s*\}/giu;
const FENCED_CODE_BLOCK = /```(?:javascript|js|typescript|ts)?[^\n]*\n[\s\S]*?```/giu;
...
function truncateAtInternalCode(text: string): string {
  const markers = [
    /\{\s*"name"\s*:\s*"codemode_execute"/iu,
